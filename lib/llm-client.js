...
/**
 * İlk analizi ELEŞTİREL gözle kontrol eden ikinci bir model çağrısı.
 * "Bu analiz yanlış olabilir mi, gözden kaçan bir şey var mı" diye sorar.
 * @param {object} product - ham ürün verisi (analiz için kullanılan aynı veri)
 * @param {object} analysis - analyzeProductWithLLM()'in döndürdüğü sonuç
 * @param {string} apiKey
 * @param {string} lang - "tr" | "en"
 */
async function verifyAnalysisWithLLM(product, analysis, apiKey, lang = "tr") {
  const langName = LANG_NAMES[lang] || LANG_NAMES.tr;
  const systemPrompt = `Sen bir kalite kontrol uzmanısın. Sana bir ürünün ham verisi VE başka
bir yapay zekanın bu ürün hakkında yaptığı analiz verilecek. Görevin bu analizi ELEŞTİREL
gözle incelemek:
- Analiz, ham verilerle (fiyat, puan, yorumlar) tutarlı mı?
- Abartılı, spekülatif ya da veriyle desteklenmeyen bir iddia var mı?
- Yorumlarla çelişen ya da göz ardı edilmiş önemli bir nokta var mı?
Kendi kendine "neden bu analiz yanlış olabilir?" diye sorarak eleştirel düşün.
Cevabını SADECE ${langName} dilinde yaz.
Görevin, SADECE aşağıdaki JSON şemasına uyan bir yanıt üretmek (başka hiçbir metin ekleme):
{
  "confidence": "high" | "medium" | "low",
  "concerns": ["varsa, analizde şüpheli/riskli bulduğun somut bir nokta", "..."],
  "correction": "varsa önemli bir düzeltme ya da ek not; sorun yoksa boş bırak"
}`;

  const userContent = `Ürün verisi:\n${JSON.stringify(product, null, 2)}\n\nDeğerlendirilecek analiz:\n${JSON.stringify(analysis, null, 2)}`;
  return callGemini(systemPrompt, userContent, apiKey);
}
